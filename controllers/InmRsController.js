import {  getKkt, getKpapd, getKpfn, getKwt, getKwv, getWtssScEmergency } from '../models/InmRsModel.js'
...
export const getScEmergency= (req, res) => {
    const schema = Joi.object({
        kode: Joi.string().allow(''),
        namaRS: Joi.string().allow('').allow(null),
        bulan: Joi.number().allow('').allow(null),
    ...
    getWtssScEmergency(req, (err, results) => {
        if (err) {
            res.status(422).send({
                status: false,
                message: err
